				...
				if((!jtf1.getText().equals(""))&&(!jtf2.getText().equals("")))
				{
					...
					case "管理员":
						try {
							...
							if(!rs.next())
								JOptionPane.showMessageDialog(null, "没有此管理员");
							else if(!(rs.getString("Manage_Password").trim().equals(jtf2.getText())))
								JOptionPane.showMessageDialog(null, "密码错误");
							else
							{
								new Manager_win(jtf1.getText());
								dispose();
							}
					...
					case "借阅":
						try{
							...
							else if(!(rs.getString("Borrower_Password").trim().equals(jtf2.getText())))
								JOptionPane.showMessageDialog(null, "密码错误");
							else
							{
								new Borrower_win(jtf1.getText());
								dispose();
							}
						}catch(SQLException e1){
							String error=e1.getMessage();
